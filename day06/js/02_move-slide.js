...
$(function() {
    ...
    // 이전 버튼 클릭 이벤트
    $('.prev').on('click', () => {
        // prependTo(부모요소) : 부모요소의 가장 앞의 자식 요소로 추가
        $('.slide-item:last-child').prependTo(list)
    ...
    $('.next').on('click', () => {
        ...
        list.animate( { left: currentPosition -= slideWidth }, 1000, function() {
         ...
         // 투명도 세팅
    $('.slide-item').stop().animate({ opacity: 0.3 }, 1000)
    $('.slide-item:nth-child(' + (initNum+1) + ')').stop().animate({ opacity : 1 }, 1000)
    ...
    $(document).on('click','.dot', function() {
        // $(this) : 여러 요소 선택지로 지정된 요소 중, 이벤트가 발생한 현재 요소
        let index = $(this).index()         //순서번호를 반환 : 0~(N-1)
        let gap = index - currentIndex      // 선택한 index - 현재 활성화된 index
        let absGap = Math.abs(gap)         // gap 의 절댓값
        

        // 활성화된 위치보다 뒤의 이미지를 선택한 경우 (prev방향)
        if( gap < 0 ) {
